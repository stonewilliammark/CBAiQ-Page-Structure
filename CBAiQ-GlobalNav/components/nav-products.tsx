...
export function NavProducts({
  ...
  return (
    <SidebarGroup className={`group-data-[collapsible=icon]:hidden ${mobileClass}`}>
      <SidebarGroupLabel>Products</SidebarGroupLabel>
      <SidebarMenu>
        {products.map((item) => (
          <SidebarMenuItem key={item.name}>
            <SidebarMenuButton asChild className={isMobile ? "mobile-menu-button" : ""}>
              <Link href={item.url}>
                <item.icon />
                <span>{item.name}</span>
              </Link>
            ...
            {item.items?.length ? (
              <>
                ...
                <SidebarMenuAction
                  ...
                  className={`transition-transform duration-200 ${openProducts[item.name] ? "rotate-90" : ""}`}
                >
                  <ChevronRight />
